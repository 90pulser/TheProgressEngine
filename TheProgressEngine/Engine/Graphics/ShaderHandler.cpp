...
void ShaderHandler::CreateProgram(const std::string& shaderName_, 
	...
	const std::string& fragmenShaderFileName_){
	std::string vertexShaderCode = ReadShader(vertexShaderFileName_);
	std::string fragmentShaderCode = ReadShader(fragmenShaderFileName_);
	...
	if (!linkResult) {
		...
		DebugLog::Error("Failed to link shader " + shaderName_ + ". Error: " 
...
std::string ShaderHandler::ReadShader(const std::string& filePath_)
{
	...
	try {
		...
		DebugLog::Error("Could not read the shader: " + filePath_,
	...
	const std::string& shaderName_)
{
	...
	if (!compileResult) {
		...
		DebugLog::Error("Error compiling shader " + shaderName_ + ". Error: \n" +
